...
When a connection needs a runtime plugin, the macOS app, Control UI, and remote guided setup show the
...
Declining, cancelling, or losing the interactive setup session stops that attempt without selecting
another inference route. A definitive model probe failure can still try the next detected option.
You can retry or choose a different connection. Installation
