...
The changed-target PR plan uses one Node test registration. Broad-risk PRs retain the metadata-complete compact fallback; canonical pushes use the integration compact. Their nondist registration counts follow the current measured weights. The `github` planner permits up to 112 hosted rows, with at most 96 running concurrently. The `hybrid` planner retains its 96-row cap; those rows consume Blacksmith registrations on attempt 1 and move to hosted capacity on retries. Screenshot-risk runs add at most two Blacksmith registrations. A measured broad PR used 43 other Blacksmith lanes. Combining those with the unchanged 96-row cap gives 139 registrations per full run; even the observed peak of 19 CI runs admitted within five minutes would use 2,641 registrations if every run took that full shape, below the 6,000-registration operating target. Other repositories and worker fleets still share that budget.
