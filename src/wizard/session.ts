...
export class WizardSession {
  ...
  private setupActivation: ProtocolWizardNextResult["setupActivation"];
  ...
  private terminalResult(): WizardNextResult {
    return {
      ...
      ...(this.status === "done" && this.modelActivation
        ...
        : {}),
      ...(this.status === "done" && this.setupActivation
        ? { setupActivation: this.setupActivation }
        : {}),
    };
  }
  ...
  /** Record the complete result for an interactive inference activation. */
  setSetupActivation(activation: NonNullable<ProtocolWizardNextResult["setupActivation"]>) {
    this.setupActivation = activation;
  }
